const checkteacher = `select * from public.teachers where email=$1`;
...
JOIN
    public.quiz_questions q ON s.question_id = q.question_id
where student_id=$1`;
...
JOIN public.classes e ON c.class_id = e.class_id
...
const getclasses = `select exam_date,exam_id,id as exam_schedule_id,total_marks,exam_schedule.subject_id,exam_schedule.subject,classes.class_id,
	classes.class_name,class_subjects.class_subject_id,class_subjects.teacher_id as subj_teacher from public.exam_schedule
join public.class_subjects on class_subjects.subject_id=exam_schedule.subject_id
	join public.classes on class_subjects.class_id=classes.class_id
...
JOIN 
    public.classes c ON s.class_id = c.class_id
WHERE 
    s.class_id = $1
GROUP BY 
